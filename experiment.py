...
import py_process
import vtrace
from environments_atari import PyProcessAtari, ATARI_ACTION_SET
...
flags.DEFINE_integer('benchmark_mode', 0,
                     'Ignore the policy actions and actually execute random policy. Use this only for throughput measurements! Agent will never learn with this flag set')
...
class Agent(snt.RNNCore):
    ...
    def __init__(self, num_actions):
        ...
        with self._enter_variable_scope():
            self._core = tf.contrib.rnn.LSTMBlockCell(512)
    ...
    def _torso(self, input_):
        ...
        if FLAGS.level_name.startswith('doom_'):
            reward, _, _, frame = env_output
            frame = frame[0]
        elif FLAGS.level_name.startswith('atari_'):
            reward, _, _, frame = env_output
            frame = frame[0]
        else:
        ...
        if FLAGS.level_name.startswith('doom_') or FLAGS.level_name.startswith('atari_'):
            log.info('Not use instructions')
        else:
...
def build_actor(agent, env, level_name, action_set):
    ...
    # return the first values and not a new snapshot of the variables.ga
...
def create_environment(level_name, seed, is_test=False):
    ...
    elif level_name.startswith('atari_'):
        config = None
        p = py_process.PyProcess(PyProcessAtari, level_name, config, FLAGS.num_action_repeats, seed)
        return environments.FlowEnvironment(p.proxy)
    else:
            ...
            if is_learner:
                ...
                while num_env_frames_v < FLAGS.total_environment_frames:
                    ...
                    for level_name, episode_return, episode_step in zip(
                        ...
                        log.info('Level: %s Episode return: %f',
                                        level_name, episode_return)
...
def main(_):
    ...
    elif level_names[0].startswith('atari_'):
        action_set = ATARI_ACTION_SET
    else:
